...
!!
    ...
    if ( use_hydrstress .and. nvegwcs /= 4 )then
        call endrun(msg='Error:: the Plant Hydraulics Stress methodology is for the spacA function is hardcoded for nvegwcs==4' &
                    //errMsg("./examples/photosynthesis/PhotosynthesisMod.f90", 769))
    end if
...
! explicitly set conductance to zero for top soil layer
    ...
    if ( nvegwcs /= 4 )then
        call endrun(subgrid_index=p, subgrid_level=subgrid_level_patch, &
            msg='Error:: this function is hardcoded for 4x4 matrices with nvegwcs==4'//errMsg("./examples/photosynthesis/PhotosynthesisMod.f90", 4735))
    end if
